...
| [next-permutation](next-permutation) | Functional recast of in-place next-permutation: `findPivot` splits `pre ++ p :: suf` at the rightmost ascent, `swapLast` swaps in the rightmost element `> p`, then reverse the suffix. Lean proves the result is a `List.Perm` of the input, strictly greater in `List.Lex (· < ·)`, **and** the lexicographic *least* such — no permutation lies strictly between input and result (`nextPerm_least`, the betweenness bound); Idris/Agda total with the split-recovery & `p < g` lemmas |
