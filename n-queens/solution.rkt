...
(module+ test
  (require rackunit)
  ;; distinct-solution counts for the n-queens problem
  (check-equal? (length (solve-n-queens 1)) 1)
  (check-equal? (length (solve-n-queens 4)) 2)
  (check-equal? (length (solve-n-queens 5)) 10)
  (check-equal? (length (solve-n-queens 6)) 4)
  ;; every board is n strings of length n
  (check-true (for/and ([b (in-list (solve-n-queens 4))])
                (and (= (length b) 4)
                     (for/and ([row (in-list b)]) (= (string-length row) 4))))))
